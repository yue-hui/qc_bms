...
<script>
import {
  get_user_analysis_child_increased,
  get_user_analysis_child_increased_bind_device,
  get_user_analysis_child_increased_bind_user
...
export default {
  ...
  methods: {
    ...
    build_xzyh: function() {
      ...
      get_user_analysis_child_increased(options).then(res => {
        this.options = {
          columns: ['日期', '新增注册用户'],
          rows: res.data.map(r => {
            return {
    ...
    build_xzsb: function() {
      const options = this.condition
      get_user_analysis_child_increased_bind_user(options).then(res => {
        this.options = {
          columns: ['日期', '新增绑定用户'],
          rows: res.data.map(r => {
            return {
    ...
    build_bd_yh: function() {
      const options = this.condition
      get_user_analysis_child_increased_bind_device(options).then(res => {
        this.options = {
          columns: ['日期', '新增绑定设备'],
          rows: res.data.map(r => {
            return {
